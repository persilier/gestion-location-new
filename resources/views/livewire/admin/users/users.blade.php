                ...
                <div class="box-body space-y-2">
                    ...
                    <div class="overflow-hidden table-bordered">
                            ...
                            <table class="table whitespace-nowrap min-w-full">
                                <thead class="bg-primary/10">
                                    <tr class="border-b border-primary/10 uppercase">
                                        <th wire:click='doSort("username")' scope="col" class="text-start">
                                            <x-datatable-item columnName="username" :sortField="$sortField"
                                        ...
                                        <th wire:click='doSort("statut")' scope="col" class="text-start">
                                            <x-datatable-item columnName="statut" :sortField="$sortField"
                                                :sortDirection="$sortDirection" />
                                        </th>
                                        <th wire:click='doSort("role")' scope="col" class="text-start">
                                            <x-datatable-item columnName="role" :sortField="$sortField"
                                                :sortDirection="$sortDirection" />
                                        </th>
                                    ...
                                    @if ($users !== 0)
                                        @foreach ($users as $user)
                                            <tr class="border-b border-primary/10">
                                                ...
                                                <td>
                                                    <span
                                                        class="badge !rounded-full bg-secondary/10 text-secondary"">{{ $user->email }}</span>

                                                </td>
                                                <td>
                                                    <span
                                                        class="badge !rounded-full bg-danger/10 text-danger"">{{ $user->tel }}</span>

                                                </td>
                                                <td><span
                                                        class='badge uppercase {{ $user->statut === 'actif' ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger' }}'>{{ $user->statut }}</span>
                                                </td>
                                                <td>
                                                    @if ($user->getRoleNames())
                                                        @foreach ($user->getRoleNames() as $role)
                                                            <span
                                                                class="badge bg-success/10 uppercase text-success">{{ $role }}</span>
                                                        @endforeach
                                                    @endif
                                                ...
                                                <td>
                                                    <div class="hstack flex gap-3 text-[.9375rem]">
                                                        <button type="button"
                                                            wire:click="$dispatch('openModal', { component: 'admin.users.user-show-modal',  arguments: { user: {{ $user->id }} } })"
                                                            class="ti-btn ti-btn-sm ti-btn-success !rounded-full"><i
                                                                class="ri-list-settings-line"></i></button>
                                                        <button
                                                            wire:click="$dispatch('openModal', { component: 'admin.users.user-modal',  arguments: { user: {{ $user->id }} } })"
                        ...
                        <div class="flex items-center w-full justify-between my-3">
                            ...
                            @endif
                            <div>
                                {{ $users->links('pagination::new-pagination') }}
                            </div>
